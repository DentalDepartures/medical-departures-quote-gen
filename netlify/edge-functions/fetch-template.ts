    ...
    if (isDriveUrl(templateUrl)) {
      ...
      if (saJson) {
        ...
        if (driveRes.status === 403) {
          return jsonError(
            'Template PDF is not accessible. Share the "Quotes Auto" Google Drive folder with quote-generator@quote-generator-495310.iam.gserviceaccount.com as Viewer.',
            403,
          )
        }
